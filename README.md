...
| 다수 클래스 | 0.195 | −0.133 |
| 어휘 규칙 | 0.285 | −0.043 |
| TF-IDF + 로지스틱 회귀 | 0.334 | +0.006 |
| **한국어 SBERT + 로지스틱 회귀** | **0.376** | **+0.048** |

**기준선은 무작위 추측이다.** 그 자로 재면 TF-IDF는 아무것도 학습하지 못했고, 표현을 한국어 문장 임베딩으로 바꾸고서야 우연을 넘었다.

측정 도중 평가셋에서 라벨 노이즈를 발견해 골든셋과 학습셋을 다시 정제하기도 했다. 그때는 점수가 오를 줄 알았는데 떨어졌다 — 이유는 [results/](results/)에 적었다.
...
src/baselines/               분류기 — majority / lexicon / embed_classifier / sbert_classifier
```
...
라벨링을 마친 뒤 `src/review.py`로 규칙 1에 걸릴 만한 것만 다시 본다. 실제로 이 단계에서 광고 댓글과 브랜드 조롱 13건이 걸러졌다.
